...
3. **Options** — annual consumption (m³/yr, default 80). Flemish
   ...
   - **Read the project's card archive** — on by default. When a
     refresh fails and the entry has nothing to serve (a restart while
     the utility's site is down), or what it holds has gone stale, the
     last card the project's daily archive holds for your utility and
     commune is served instead of failing setup or serving a card the
     archive has already replaced. The request to GitHub names the
     utility and the commune id and nothing else; untick the box and the
     integration never contacts GitHub.
...
dated the first day the archive saw that card in its month, so the
35-day staleness clock runs from there, with the failure in
`last_error`. A row is rewritten only when the card changes, so that
date can be up to a month older than the archive's last sighting, and
