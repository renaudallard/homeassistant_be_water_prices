...
| **AIEC** | Wallonia (Condroz) | small | [`providers/aiec.py`](./custom_components/be_water_prices/providers/aiec.py) — the CVD comes from `callmepower.be/fr/eau/distributeurs/aiec` (public aggregator) because AIEC publishes its card only as a picture. The picture's name carries the day it took effect (`Tarif-2026-04-1-*.jpg`), so each fetch also reads `eauxducondroz.be/Prix.htm` and dates the card from it, since the aggregator prints no date and every card would otherwise be stamped 1 January. The rate itself is still the aggregator's, and on 2026-09-08 it was 2,460 against the operator's 3,050 |
...
3. **Options** — annual consumption (m³/yr, default 80). Flemish
   ...
   - **Commune** *(optional)* — pick your specific commune to get
     ...
     pre-selects that commune for you: Water-link's 2070, Pidpa's
     Nijlen, Wommelgem and Kasterlee, and Farys's Drogenbos and four
     Zaventem postcodes. Everywhere else the default is
