...
| **AIEC** | Wallonia (Condroz) | small | [`providers/aiec.py`](./custom_components/be_water_prices/providers/aiec.py) — the CVD comes from `callmepower.be/fr/eau/distributeurs/aiec` (public aggregator) because AIEC publishes its card only as a picture. The picture's name carries the day it took effect (`Tarif-2026-04-1-*.jpg`), so each fetch also reads `eauxducondroz.be/Prix.htm` and refuses the aggregator when the operator has already published a later card |
